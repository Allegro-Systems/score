...
private func readResponsePart(from channel: EmbeddedChannel) throws -> HTTPServerResponsePart {
    for _ in 0..<50 {
    ...
    throw ResponseCaptureError.missingPart
}

@Test func requestHandlerRendersPagesForGetRequests() throws {
    let response = try performRequest(method: NIOHTTP1.HTTPMethod.GET, uri: "/?debug=1")
...
@Test func requestHandlerInvokesControllerHandlers() throws {
    let response = try performRequest(method: NIOHTTP1.HTTPMethod.GET, uri: "/api/echo/value-42")
...
@Test func requestHandlerUsesFallbackForUnknownHTTPMethods() throws {
...
@Test func requestHandlerReturnsInternalServerErrorWhenControllerThrows() throws {
    let response = try performRequest(method: NIOHTTP1.HTTPMethod.GET, uri: "/api/boom")
...
@Test func requestHandlerReturnsPlainOkForRoutesWithoutPageOrHandler() throws {
    let response = try performRequest(method: NIOHTTP1.HTTPMethod.GET, uri: "/api/noop")
...
@Test func requestHandlerReturnsNotFoundForUnknownPaths() throws {
    let response = try performRequest(method: NIOHTTP1.HTTPMethod.GET, uri: "/missing")

    #expect(response.status == HTTPResponseStatus.notFound)
    #expect(response.body == "Not Found")
}

@Test func requestHandlerReturnsMethodNotAllowedWithAllowHeader() throws {
    let response = try performRequest(method: NIOHTTP1.HTTPMethod.DELETE, uri: "/api/echo/value-42")
...
@Test func requestHandlerPassesQueryParametersToHandler() throws {
    let response = try performRequest(method: NIOHTTP1.HTTPMethod.GET, uri: "/api/query?name=Alice")

    #expect(response.status == HTTPResponseStatus.ok)
    #expect(response.body == "Alice")
}

@Test func requestHandlerPassesRequestBodyToHandler() throws {
    var buffer = ByteBuffer()
    buffer.writeString("{\"key\":\"value\"}")
    let response = try performRequest(method: NIOHTTP1.HTTPMethod.POST, uri: "/api/body", body: buffer)

    #expect(response.status == HTTPResponseStatus.ok)
    #expect(response.body == "{\"key\":\"value\"}")
}

@Test func requestHandlerRespectsResponseStatusAndHeaders() throws {
    let response = try performRequest(method: NIOHTTP1.HTTPMethod.POST, uri: "/api/status")
